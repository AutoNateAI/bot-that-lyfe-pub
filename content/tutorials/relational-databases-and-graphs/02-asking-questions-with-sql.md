...
Nate and Kai are building a studio, Bot That Lyfe, and they've just finished putting their real operational records into a real database — three tables in a file called `studio.db`. `people` holds everyone they've met at the Fairview Founders Table meetup. `ideas` holds the running backlog. `feedback` holds what a specific person said about a specific idea, on a specific night, linked to both by foreign keys. Two weeks of back-filling later, they had about ninety rows of feedback in there.

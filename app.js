...
// No commerce backend is deployed for this publication (commerce lives at
// shop.botthatlyfe.com) — this only matters to the unused checkout/booking/
// sponsorship routes inherited from the codebase this repo forked from,
// none of which are wired into a live nav path. Point this at a real API
// before enabling any of that code.
const marketplaceApiBase = "";
...
workWithUsForm?.addEventListener("submit", (event) => {
  ...
  const subject = `Work With Bot That Lyfe — ${fields.orgType || "Inquiry"} from ${fields.name}`;
  const bodyLines = [
    `Name: ${fields.name}`,
    `Email: ${fields.email}`,

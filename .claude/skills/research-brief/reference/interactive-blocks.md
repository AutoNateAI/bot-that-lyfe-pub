...
- `sourceLabel` renders as a `<figcaption>` under the chart — **always fill this in**. It's the difference between a chart and a claim; say exactly what years/geography/units the numbers cover, and flag it explicitly if you're putting two non-comparable series side by side for scale illustration only.
...
```map
{
  ...
  "zoom": 9,
  "markers": [
    { "lat": 42.2780, "lng": -83.7382, "label": "University of Michigan — why this point matters" }
  ],
...
- Every marker needs real, verifiable `lat`/`lng` — a campus, a lab building, a company HQ. Never a precise coordinate for a private address or an invented location. Two or more markers auto-fit the map bounds.
...
```graph
{
  ...
  "nodes": [
    { "id": "grant", "label": "NSF Award\n($4.2M)", "rank": 0 },
    { "id": "lab", "label": "Autonomous Systems Lab", "rank": 1 },
    { "id": "hiring", "label": "Research Engineer Hiring", "rank": 2 },
    { "id": "openroles", "label": "Open Undergrad Roles", "rank": 3, "output": true, "baseline": 3, "format": "", "driverInput": "grant", "driverGain": 0.02 }
  ],
  "edges": [
    { "from": "grant", "to": "lab", "evidence": "verified", "label": "NSF award database: $4.2M, 3-year term" },
    { "from": "lab", "to": "hiring", "evidence": "estimated", "label": "Grant funds typically cover 2-4 research engineer/RA positions" },
    { "from": "hiring", "to": "openroles", "evidence": "hypothesis", "label": "Undergrad-accessible role count not yet confirmed with the lab directly" }
  ],
  "inputs": [
    { "id": "grant", "label": "Award size, indexed to the disclosed amount", "min": 0, "max": 150, "step": 5, "default": 100, "unit": "", "verifiedAt": 100, "verifiedNote": "100 = the actual $4.2M disclosed award" }
  ],

# Bot That Lyfe Social Distribution Setup

**Heads up (2026-09-20 fork note):** the `.env` values copied into this repo from the AutoNateAI codebase point `FACEBOOK_PAGE_ID`/`FACEBOOK_PAGE_ACCESS_TOKEN` at the **AutoNate AI** Facebook Page (see "Verified Test" below — that's the page these credentials actually post to right now), not a Bot That Lyfe page. The LinkedIn app (`Bot That Lyfe Publisher`) and Meta app (`Bot That Lyfe Page Publisher`) are already named for this brand, but the Facebook Page itself needs to be swapped to a real Bot That Lyfe Page before this goes live — regenerate the Page token against the right Page and update `.env`.

This repo can publish reviewed research links to Facebook Pages and LinkedIn from a local CLI script:

```bash
npm run social:post -- --platform facebook --message "Test post" --link "https://botthatlyfe.com/research-and-case-studies"
```

The command is a dry run by default. Add `--confirm` only when you are ready to publish:

```bash
npm run social:post -- --platform facebook --message "Test post" --link "https://botthatlyfe.com/research-and-case-studies" --confirm
```
...
For the Bot That Lyfe setup, the app was:

```text
Bot That Lyfe Page Publisher
```
...
For broader local testing, Bot That Lyfe also enabled:
...
```text
Meta App: Bot That Lyfe Page Publisher
...
npm run social:post -- \
  --platform facebook \
  --message "Testing the Bot That Lyfe research publishing workflow. We are connecting the research site to distribution so future university-intelligence briefs can move from investigation to publication to outreach." \
  --link "https://botthatlyfe.com/research-and-case-studies" \
  --confirm
```
...
The current Bot That Lyfe LinkedIn workflow posts from Nathan's personal/member profile, not the company Page. This is intentional: research distribution starts from the trusted human account, with the company Page as a later amplification channel.
...
npm run social:post -- \
  --platform linkedin \
  --message "Testing the Bot That Lyfe research publishing workflow. Full research library: https://botthatlyfe.com/research-and-case-studies" \
  --link "https://botthatlyfe.com/research-and-case-studies"
```
